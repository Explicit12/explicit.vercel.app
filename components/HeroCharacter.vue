<script lang="ts" setup>
  ...
  function addCharacterMouseTracking(neck: Object3D, head: Object3D) {
    document.addEventListener("mousemove", (event) => {
      moveJoint(event, neck, 25);
      moveJoint(event, head, 25);
    });
  }
  ...
  onMounted(() => {
    ...
    addCharacter().then(({ characterModel, animationMixer }) => {
      const head = characterModel.scene.children[0].getObjectByName("Head");
      const neck = characterModel.scene.children[0].getObjectByName("Neck");
      if (head && neck) addCharacterMouseTracking(neck, head);
